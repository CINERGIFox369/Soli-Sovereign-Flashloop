...
const server = http.createServer((req, res) => {
  if (req.url === '/' || req.url === '/dashboard') {
    const html = require('fs').readFileSync(path.join(__dirname, 'dashboard.html'));
...
function startSimulator() {
  // spawn node with ts-node loader
  const child = spawn('node', ['--loader', 'ts-node/esm', SIM_PATH], { env: { ...process.env, LIVE: '1' } });
  child.stdout.setEncoding('utf8');
  child.stdout.on('data', (data) => {
    // simulator prints JSON per-line when LIVE=1
