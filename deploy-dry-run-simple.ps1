...
Write-Host "`nSetting up Container Apps environment..." -ForegroundColor Green
try {
    ...
    if (-not $envExists) {
        ...
        az containerapp env create --name $ENVIRONMENT_NAME --resource-group $RESOURCE_GROUP --location eastus --logs-workspace-id $lawId
        Write-Host "Container Apps environment created successfully"
    } else {
...
Write-Host "`nDeploying container app in DRY RUN mode..." -ForegroundColor Green
try {
    ...
    if ($appExists) {
        ...
        az containerapp create --name $CONTAINER_APP_NAME --resource-group $RESOURCE_GROUP --environment $ENVIRONMENT_NAME --image $IMAGE_NAME --registry-server "$ACR_NAME.azurecr.io" --cpu 1.0 --memory 2.0Gi --min-replicas 1 --max-replicas 3 --env-vars "DRY_RUN=true" "AZURE_KEY_VAULT_URL=https://$KEY_VAULT_NAME.vault.azure.net/" "NETWORK_ID=42161" "RPC_URL=https://arb1.arbitrum.io/rpc" "MIN_EDGE_BPS=150" "MAX_DAILY_LOSS=100" "AAVE_LOAN_FRACTION=0.01" "CONFIRM_REAL=false"
    }
} catch {
    Write-Host "Error deploying container app: $_" -ForegroundColor Red
}

# Step 4: Configure managed identity and Key Vault permissions
Write-Host "`nConfiguring managed identity and Key Vault access..." -ForegroundColor Green
try {
    ...
    Write-Host "Error configuring managed identity: $_" -ForegroundColor Red
}

# Step 5: Display deployment summary
Write-Host "`nDry Run Deployment Complete!" -ForegroundColor Cyan
...
Write-Host "`nMonitor the deployment:" -ForegroundColor Yellow
Write-Host "az containerapp logs show --name $CONTAINER_APP_NAME --resource-group $RESOURCE_GROUP --follow" -ForegroundColor Gray

Write-Host "`nIMPORTANT: This is a DRY RUN - no real transactions will be executed!" -ForegroundColor Red
